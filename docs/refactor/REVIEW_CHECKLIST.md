...
> 검수자: 사용자 또는 Claude(계획자). 통과 못 하면 머지 금지.
...
### C-1. 오케스트레이터 패턴 (Task 02·03·06·10·11·12 등 — 현재는 옵션, DEC-WORKFLOW-010)
- [ ] **모든 출력은 실제 실행 결과** (실행자 또는 사용자가 날조한 출력 0건)
- [ ] 실행 못 한 항목은 `<실행 불가: 사유>`(실행자) 또는 `<미제공>`(사용자) 표기
- [ ] 명령 결과 spot check 1건 이상 (계획자가 WSL에서 재실행해 일치 확인 — 셸 환경 불확정 대비)

### C-2. Claude 직접 실행 패턴 (Task 05·07 등)
- [ ] 작업지시서 본문(§6 골격)과 결과 파일이 1:1 대응
- [ ] Claude가 임의로 추가·삭제·변형한 부분 없음 (현행 사실 반영 목적의 정정은 PR에 명시)

### C-3. 실행자(refactor-executor) 일반 패턴
- [ ] 출력이 작업지시서 acceptance를 모두 다룸 (lost-in-the-middle 의심 점검)
- [ ] HARD LIMIT 외 파일에 변경이 들어가지 않았는지 diff 재확인
- [ ] 빌드 검증(`compileJava`/`type-check`) 결과가 실제 실행 (계획자 재확인 가능)
