...
> PRINCIPLES.md §13 (실행자 직접 실행 — DEC-WORKFLOW-010):

현 실행자(Sonnet)는 grep·비교를 직접 실행·검증 가능. 받은 실제 출력만 기록(날조 금지).

## 6. 작업 절차 (실행자 직접 실행 — DEC-WORKFLOW-010)

> **실행자(refactor-executor)가 SYNC-01~04의 grep을 WSL에서 직접 실행**하고 실제 출력만 기록. 아래 "Copilot 발화/사용자" 표현은 무시하고 명령만 실행.
> **SYNC-05(기획서↔구현)는 수동 비교** — 실행자가 `md/TeamBlackbox_기획서_v3.md`와 코드를 읽고 best-effort 판단. 확신 가능하면 N/Y 기록(Spec/State 1차), **불확실하면 D4(사용자 결정 필요)로 기록**(억지 단정 금지). INV-06 결과(ScoreService 괴리)는 SYNC-05-1과 연계.
> 환경: **WSL Ubuntu bash**. 출력 날조 금지.
...
- **실행자가 grep·비교를 직접 실행**하고 실제 결과만 기록 (SYNC-05 불확실 시 D4 — 날조 금지).

## 8. 검수 기준 (Acceptance Criteria)

- [ ] DRIFT_INVENTORY.md SYNC 섹션 채워짐
- [ ] **모든 행 데이터는 실행자 실제 실행/판단 기반** (날조 0; SYNC-05 불확실은 D4)
