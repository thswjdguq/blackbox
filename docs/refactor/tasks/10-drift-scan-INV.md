...
현 실행자(Sonnet)는 grep을 직접 실행·검증 가능. 받은 실제 출력만 기록(날조 금지).

## 6. 작업 절차 (실행자 직접 실행 — DEC-WORKFLOW-010)

> **실행자(refactor-executor)가 아래 각 검사의 grep을 WSL에서 직접 실행**하고 실제 출력만 표/로그에 기록한다.
> 아래 블록의 "Copilot 발화/사용자에게 요청" 표현은 **무시**하고, grep 명령만 그대로 실행하면 된다. 출력 날조 금지.
> 환경: **WSL Ubuntu bash** (PRINCIPLES §13).
> **해석 유예:** INV-01·04·06·07은 grep만으로 위반 확정이 어려움 → 원시 grep 출력 + "후보(분류 Task 13 유예)"로 기록. INV-02·05는 패턴이 명확(비면 none).
...
- [x] **Skip** — 절차 6-1~6-9 명확. 코드 변경 없음(문서 1개 생성).
...
- [ ] **모든 행 데이터는 실행자가 실제 실행한 grep 결과 기반** (날조 0)
