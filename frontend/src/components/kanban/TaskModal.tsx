...
import { X, Trash2, Save, Plus, ChevronDown, ChevronUp, Link2, Unlink } from "lucide-react";
...
interface TaskModalProps {
  ...
  task: Task | null;
  members: Member[];
  projectId: string; // 제출물 목록 조회에 필요
...
export default function TaskModal({
  ...
  const [delLoadFailed,     setDelLoadFailed]      = useState(false); // 실패 시 섹션 숨김
  const [showDelSection,    setShowDelSection]     = useState(false); // 접기/펼치기
  const [selectedDelId,     setSelectedDelId]      = useState<string>(task?.deliverableId ?? "");
  const [selectedReqId,     setSelectedReqId]      = useState<string>(task?.requirementId ?? "");
  const [completionCriteria, setCompletionCriteria] = useState(task?.completionCriteria ?? "");

  // 선택한 제출물의 요구사항 목록
  const selectedDeliverable = deliverables.find((d) => d.id === selectedDelId);

  // 제출물 섹션 열 때 목록 조회
  useEffect(() => {
    if (!showDelSection || deliverables.length > 0) return;
    setDelLoading(true);
    setDelLoadFailed(false);
    getDeliverables(projectId)
      .then((res) => setDeliverables(res.data))
      .catch(() => setDelLoadFailed(true))
      .finally(() => setDelLoading(false));
  }, [showDelSection, deliverables.length, projectId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload: CreateTaskPayload = {
        ...
        status,
        // 제출물 연결 필드 — 선택값이 있으면 전송, 없으면 생략(기존 값 유지)
        ...(selectedDelId
          ? { deliverableId: selectedDelId }
          : task?.deliverableId
          ? { clearDeliverable: true }  // 기존 연결이 있었는데 비워졌으면 해제
          : {}),
        ...(selectedReqId
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg bg-slate-800 border border-slate-700 rounded-2xl shadow-2xl shadow-black/50 overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4 max-h-[80vh] overflow-y-auto">
          ...
          {/* 제출물 연결 섹션 — 로드 실패 시 전체 숨김 (모달 블로킹 방지) */}
          {!delLoadFailed && (
            <div className="border border-slate-700 rounded-lg overflow-hidden">
              {/* 접기/펼치기 헤더 */}
              <button
                type="button"
                onClick={() => setShowDelSection((v) => !v)}
                className="w-full flex items-center justify-between px-4 py-3 text-xs font-medium text-slate-400 hover:text-slate-200 hover:bg-slate-700/40 transition-colors"
              >
                <span className="flex items-center gap-1.5">
                  <Link2 size={13} />
                  제출물 연결
                  {/* 현재 연결된 제출물 이름을 미리 보여줌 */}
                  {selectedDelId && selectedDeliverable && (
                    <span className="ml-1 text-indigo-400">
                      — {selectedDeliverable.title}
                    </span>
                  )}
                </span>
                {showDelSection ? <ChevronUp size={13} /> : <ChevronDown size={13} />}
              </button>
              ...
              {showDelSection && (
                <div className="px-4 pb-4 pt-1 space-y-3 bg-slate-900/40">
                  ...
                  {/* 제출물 0건 */}
                  {!delLoading && deliverables.length === 0 && (
                    <p className="text-xs text-slate-500 py-2">연결할 제출물이 없습니다.</p>
                  )}
                  ...
                  {!delLoading && deliverables.length > 0 && (
                    <>
                      <div>
                        <label className="block text-xs text-slate-400 mb-1.5">제출물</label>
                        <select
                          value={selectedDelId}
                          onChange={(e) => {
                            setSelectedDelId(e.target.value);
                            setSelectedReqId(""); // 제출물 바뀌면 요구사항 초기화
                          }}
                          className={INPUT_CLS}
                        >
                      ...
                      {selectedDeliverable && (
                        <div>
                          <label className="block text-xs text-slate-400 mb-1.5">요구사항 (선택)</label>
                          <select
                            value={selectedReqId}
                          ...
                          >
                            <option value="">선택 안 함</option>
                            {selectedDeliverable.requirements.map((r) => (
                              <option key={r.id} value={r.id}>
                                {r.required ? "[필수] " : "[선택] "}{r.content}
                              </option>
                            ))}
                          </select>
                        </div>
                      ...
                      {/* 완료 기준 */}
                      <div>
                        <label className="block text-xs text-slate-400 mb-1.5">완료 기준</label>
                        <input
                          type="text"
                          value={completionCriteria}
                          onChange={(e) => setCompletionCriteria(e.target.value)}
                          placeholder="이 업무가 완료되는 조건"
                          className={INPUT_CLS}
                        />
                      </div>

                      {/* 연결 해제 버튼 — 현재 연결이 있을 때만 */}
                      {(selectedDelId || task?.deliverableId) && (
                        <button
                          type="button"
                          onClick={() => {
                            setSelectedDelId("");
